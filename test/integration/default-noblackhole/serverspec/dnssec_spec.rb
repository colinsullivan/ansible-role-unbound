...
describe command('dig www.cloudflare.com. A +dnssec +multiline'), :if => os[:family] == 'ubuntu' || os[:family] == 'debian' do
  its(:stdout) { should match /ad/ }
  its(:stdout) { should match /flags: do/ }
  its(:stdout) { should match /RRSIG/ }
end
describe command('dig www.cloudflare.com. A +dnssec +multiline'), :if => os[:family] == 'redhat' do
## why???
#  its(:stdout) { should match /ad/ }
  its(:stdout) { should match /flags: do/ }
  its(:stdout) { should match /RRSIG/ }
